...
nocache('../message/help.js', module => console.log(color('[WATCH]', 'cyan'), color(`'${module}'`, 'green'), 'File is updated!'))
	...
	console.log(color(figlet.textSync('BOSCOTR BY LAÇİN EKE', {
		font: 'Standard',
	...
	console.log(color('[LAÇİN]', 'cyan'), color('HOŞGELDİNİZ!', 'white'))
	console.log(color('[EKE]', 'cyan'), color('BOT ÇALIŞIYOR!', 'yellow'))
	bosco.browserDescription = ["ABONE OL YTB - [LAÇİN - EKE]", "firefox", "3.0.0"];
 ...
 teks = `https://chat.whatsapp.com/BqwU4hCuq0d3AOTerPVatJ`
 bosco.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('QR ÇALIŞIYOR!', 'blue'))
	...
	bosco.on('group-participants-update', async (anu) => {
		...
		if (anu.action == 'promote'){
			...
			try {
					...
					ppimg = 'https://telegra.ph/file/97e1a5d6d36e2853a977d.jpg'
				}
...
`
   ...
   remoteJid: "0@s.whatsapp.net"},message: {"groupInviteMessage": {"groupJid": "6288213840883-1616169743@g.us","inviteCode": "mememteeeekkeke","groupName": "Alphabot", "caption": `🌹 𝑷 𝑬 𝑷 𝑬 - 𝑺 𝑬 𝑹 🌹`, 'jpegThumbnail': fs.readFileSync(`ds.jpg`)}}}
    ...
    teks = `*[ Gʀup Kapandı! ]*\n`
    bosco.sendMessage(metdata.id, teks, MessageType.text, {quoted: falfa})
    console.log(clc.yellow(`[ Grup Kapandı! ] In ${metdata.subject}`))
  } else if(!anu.desc == ''){
    tag = anu.descOwner.split('@')[0] + '@s.whatsapp.net'
    teks = `*[ Gʀup açıklaması değişti! ]*\n\n • *yeni* : *${anu.desc}*`
    bosco.sendMessage(metdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [tag]}, quoted: falfa})
    console.log(clc.yellow(`[ Grup açıklaması değişti! ] In ${metdata.subject}`))
 } else if(anu.restrict == 'false'){
    teks = `*[ Gʀup ayarları değişti! ]*\n`
    bosco.sendMessage(metdata.id, teks, MessageType.text, {quoted: falfa})
    console.log(clc.yellow(`[ Grup ayarları değişti! ] In ${metdata.subject}`))
